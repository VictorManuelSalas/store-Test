  ...
  <div>
    ...
    <v-card :loading="loading" class="ma-1 pb-10" max-width="100%">
      ...
      <section id="avatar-section">
        <div>
          <v-btn id="btn-pass" outlined @click="changePassword">
        ...
        <div>
          ...
          <v-card-title class="text-h4">{{ userInfo?.firstname || "user name" }}
      ...
      <v-card-text class="mx-4 pt-8">
        <v-row><v-col cols="12" sm="4" xl="6">Account ID (Odoo):
            {{ userInfo?.accountID.join(", ") || "-" }}</v-col>
          <v-col cols="12" sm="4" xl="6">Email: {{ userInfo?.email || "-" }}</v-col><v-col cols="12" sm="4"
            xl="6">Phone: {{
              userInfo?.phoneNumber || "-" }}</v-col><v-col cols="12" sm="4" xl="6">Role: {{ userInfo?.role || "-"
            }}</v-col>
          <v-col cols="12" sm="4" xl="6">Modified Date:
            {{ convertTimestampToDate(userInfo?.modifiedAt || "-") }}</v-col>

          <v-col cols="12" sm="4" xl="6">Created Date:
            {{ convertTimestampToDate(userInfo?.createAt || "-") }}</v-col>
          <v-col cols="12" sm="4" xl="6"><span style="display: flex">Modified By:
              <div style="width: 70%">
          ...
          <v-col cols="12" sm="4" xl="6">
            <span style="display: flex">Created By:
              <div style="width: 70%">
