  ...
  <v-card id="menu">
    <v-navigation-drawer v-model="drawer" :mini-variant.sync="mini" permanent>
      <v-list-item id="menu-header" v-if="!mini">
        <v-icon id="btn" icon @click.stop="mini = !mini">mdi-chevron-left
        </v-icon>
        <img src="../assets/smartStoreLogo.webp" alt="Logo" />
      </v-list-item>
      <div id="avatar-menu">
        <img v-if="user.avatar.length > 0" :src="user.avatar" :style="mini ? { width: '50px' } : { width: '100px' }" />
        ...
        <div>
          <span v-for="item in items" :key="item.title">
            <router-link :to="item.url">
              <v-list-item class="list-menu-items" link :style="!mini ? { gap: '10px' } : ''">
        ...
        <div>
          <v-dialog v-model="dialog" persistent max-width="290">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item v-bind="attrs" v-on="on" id="logout" link :style="!mini ? { gap: '10px' } : ''">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                  fill="#000000">
                  <path
